...
//            char[] textoCriptografado = rc4.criptografa("texto".toCharArray());
//            System.out.println("Texto Criptografado: " + new String(textoCriptografado));
//            System.out.println("Texto Decriptografado:" + new String(rc4.decriptografa(textoCriptografado)));
//            
...
// 
    ...
    public void setKey(String key) throws InvalidKeyException {
        if (!(key.length() >= TAM_MIN_CHAVE && key.length() < SBOX_LENGTH)) {
            throw new InvalidKeyException("Tamanho da chave deve ser entre "+ TAM_MIN_CHAVE + " e " + (SBOX_LENGTH - 1));
        }
